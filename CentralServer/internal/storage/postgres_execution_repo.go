...
// ListByLambda retrieves all executions for a specific lambda function
func (r *PostgresExecutionRepo) ListByLambda(ctx context.Context, lambdaID string) ([]*domain.Execution, error) {
	query := `
	SELECT id, lambda_id, reference_id, input, status, output, error, started_at, finished_at
	FROM executions
	WHERE lambda_id = $1
	...
	rows, err := r.db.QueryContext(ctx, query, lambdaID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
