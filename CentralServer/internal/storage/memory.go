...
func (r *MemoryLambdaRepo) List(ctx context.Context, userID string) ([]*gwdomain.Lambda, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	result := make([]*gwdomain.Lambda, 0, len(r.byID))
	for _, l := range r.byID {
		if userID == "" || userID == "unknown" || l.UserID == userID {
