...
type AuthState = {
  ...
  googleLogin: (token: string) => Promise<Session>;
  logout: () => Promise<void>;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const googleLoginMutation = useMutation({
    mutationFn: async (idToken: string) => {
      ...
      localStorage.setItem("auth_token", res.token);
      localStorage.setItem("auth_email", res.username || "google-user");
      localStorage.setItem("auth_expires", new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString());
      return {
        token: res.token,
        email: res.username || "google-user",
        expiresAt: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(),
      } as Session;
    },
    onSuccess: (session) => {
  ...
  const value = useMemo<AuthState>(
    () => ({
      ...
      googleLogin: async (token) => googleLoginMutation.mutateAsync(token),
      logout: async () => {
