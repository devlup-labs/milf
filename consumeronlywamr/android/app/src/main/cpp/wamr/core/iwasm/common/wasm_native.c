...
check_symbol_signature(const WASMFuncType *type, const char *signature)
{
    ...
    __android_log_print(ANDROID_LOG_ERROR, "native-lib",
                        "Checking sig '%s' against func_type=%p "
                        "(param_count=%u, result_count=%u)",
                        signature, type, type->param_count, type->result_count);
    for (uint32 k = 0; k < type->param_count; k++) {
        __android_log_print(ANDROID_LOG_ERROR, "native-lib", "Param %u type=%u",
                            k, type->types[k]);
    }

    if (!p || strlen(p) < 2) {
        LOG_ERROR("check_symbol_signature: invalid signature string: %s",
    ...
    if (*p++ != '(') {
        LOG_ERROR("check_symbol_signature: expected '(' but got '%c' in %s",
                  *(p - 1), signature);
        return false;
    }

    if ((uint32)(p_end - p) < (uint32)(type->param_count + 1)) {
        LOG_ERROR("check_symbol_signature: too short. p_end-p=%u, "
                  "param_count=%u. Sig: %s",
                  (uint32)(p_end - p), type->param_count, signature);
    ...
    for (i = 0; i < type->param_count; i++) {
        ...
        if (type->types[i] != VALUE_TYPE_I32) {
            ...
            LOG_ERROR("check_symbol_signature: param %u type %u is not I32 for "
                      "pointer/string. Sig char: %c",
                      i, type->types[i], sig);
            return false;
        }
        ...
        if (sig == '*') {
            ...
            /* invalid signature */
            LOG_ERROR("check_symbol_signature: param %u mismatch. Type %u, Sig "
                      "char '%c'. Signature: %s",
                      i, type->types[i], sig, signature);
            return false;
        }
    }

    if (*p++ != ')') {
        LOG_ERROR("check_symbol_signature: expected ')' but got '%c'. Sig: %s",
                  *(p - 1), signature);
        return false;
    }
    ...
    if (type->result_count) {
        if (p >= p_end) {
            LOG_ERROR(
                "check_symbol_signature: premature end for result. Sig: %s",
                signature);
            return false;
        }
        ...
        if (!compare_type_with_signature(type->types[i], *p)) {
            LOG_ERROR(
                "check_symbol_signature: result type mismatch. Sig char: %c",
                *p);
            return false;
        }
    ...
    if (*p != '\0') {
        LOG_ERROR("check_symbol_signature: extra chars at end: %s", p);
        return false;
    }
                           ...
                           bool *p_call_conv_raw)
{
    ...
    if (p_signature && !*p_signature && func_ptr) {
        if (signature && signature[0] != '\0') {
            if (!func_type) {
                LOG_ERROR("wasm_native_resolve_symbol: func_type IS NULL for "
                          "%s.%s! Signature: %s",
            ...
            if (!func_type || !check_symbol_signature(func_type, signature)) {
                ...
                LOG_WARNING("failed to check signature '%s' and resolve "
                            ...
                            "func_type=%p\n",
                            signature, module_name, field_name, func_type);
#endif
                return NULL;
            }
            else
...
wasm_native_set_context(WASMModuleInstanceCommon *inst, void *key, void *ctx)
{
    ...
    if (!common) {
        LOG_ERROR("wasm_native_set_context: common is NULL!");
        return;
    }
    LOG_ERROR("wasm_native_set_context: setting context for idx %u to %p", idx,
              ctx);
...
{
    if (key == NULL) {
        LOG_ERROR("wasm_native_get_context: key is NULL!");
    }
    uint32 idx = context_key_to_idx(key);
    WASMModuleInstanceExtraCommon *common = wasm_module_inst_extra_common(inst);
    if (!common) {
        LOG_ERROR("wasm_native_get_context: common is NULL!");
        return NULL;
    }
...
wasm_native_init()
{
    ...
    if (g_wasi_context_key == NULL) {
        LOG_ERROR("failed to create wasi context key");
        goto fail;
    }
    LOG_ERROR("wasi context key created: %p", g_wasi_context_key);
