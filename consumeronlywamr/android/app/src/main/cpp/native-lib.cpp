...
Java_com_example_consumeronlywamr_WasmService_invokeDataWasmNative(
    ...
    jbyteArray payload, jstring hostPath) {
  const char *nativeFuncName = env->GetStringUTFChars(funcName, NULL);
  const char *nativeHostPath = env->GetStringUTFChars(hostPath, NULL);
  char *buffer = NULL;
  char error_buf[128];
  wasm_module_t module = NULL;
  wasm_module_inst_t module_inst = NULL;
  wasm_exec_env_t exec_env = NULL;
  jbyteArray resultByteArray = env->NewByteArray(0);
  ...
  if (!buffer) {
    LOGE("Failed to allocate buffer");
    env->ReleaseStringUTFChars(hostPath, nativeHostPath);
  ...
  // Set WASI parameters (no directory mapping — isolated process can't access
  // host dirs)
  wasm_runtime_set_wasi_args(module, NULL, 0, NULL, 0, NULL, 0, NULL, 0);

  // 3. Instantiate with a large heap (64MB) for image/PDF processing
  uint32_t heap_size = 64 * 1024 * 1024;
  module_inst = wasm_runtime_instantiate(module, 8192, heap_size, error_buf,
                                         sizeof(error_buf));
  if (!module_inst) {
    LOGE("Instantiation failed: %s", error_buf);
    goto cleanup;
  ...
  // 5. Dynamic Lookup for universal invoke
  {
    ...
    if (!func) {
      LOGE("Function '%s' (or main/_start) not found in WASM module",
           nativeFuncName);
      goto cleanup;
    }

    jsize in_size = env->GetArrayLength(payload);
    void *native_in_ptr = NULL;
    uint32_t in_ptr =
        wasm_runtime_module_malloc(module_inst, in_size, &native_in_ptr);
    if (!in_ptr || !native_in_ptr) {
      LOGE("Failed to allocate input memory in WASM (size: %d). Does module "
           "export malloc?",
           in_size);
      // Return a special error byte to indicate allocation failure
      resultByteArray = env->NewByteArray(1);
      jbyte err = -1;
      env->SetByteArrayRegion(resultByteArray, 0, 1, &err);
      goto cleanup;
    }

    // copy data to WASM memory
    env->GetByteArrayRegion(payload, 0, in_size, (jbyte *)native_in_ptr);

    uint32_t out_capacity = 10 * 1024 * 1024; // 10 MB maximum output pdf size
    void *native_out_ptr = NULL;
    uint32_t out_ptr =
        wasm_runtime_module_malloc(module_inst, out_capacity, &native_out_ptr);
    if (!out_ptr || !native_out_ptr) {
      LOGE("Failed to allocate output memory in WASM (size: %d). Does module "
           "export malloc?",
           out_capacity);
      wasm_runtime_module_free(module_inst, in_ptr);
      // Return a special error byte
      resultByteArray = env->NewByteArray(1);
      jbyte err = -2;
      env->SetByteArrayRegion(resultByteArray, 0, 1, &err);
      goto cleanup;
    ...
    if (wasm_runtime_call_wasm(exec_env, func, 4, argv)) {
      ...
      LOGI("invoke executed. Returned size: %d", out_size);
      if (out_size > 0 && out_size <= out_capacity) {
        resultByteArray = env->NewByteArray(out_size);
        env->SetByteArrayRegion(resultByteArray, 0, out_size,
                                (const jbyte *)native_out_ptr);
      } else {
        LOGE("invoke returned invalid size: %d (capacity: %d)", out_size,
             out_capacity);
      }
    } else {
